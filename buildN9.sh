...
printf "Building for device: crownlte\n"
...
if [ ! -d $KERN_DIR/$KERNEL_NAME/crownlte ]; then
  mkdir -p $KERN_DIR/$KERNEL_NAME/crownlte
fi

if [ ! -d $KERN_DIR/$DTB_NAME/crownlte ]; then
  mkdir -p $KERN_DIR/$DTB_NAME/crownlte
fi

if [ ! -f $CUR_DIR/arch/arm64/boot/Image ] || [ ! -f $CUR_DIR/arch/arm64/boot/dtb.img ]; then
  printf "Operation completed with errors!\n"
else
  cp -vr $CUR_DIR/arch/arm64/boot/Image $KERN_DIR/$KERNEL_NAME/crownlte/zImage
  cp -vr $CUR_DIR/arch/arm64/boot/dtb.img $KERN_DIR/$DTB_NAME/crownlte/dtb.img
  printf "Success!\n"
fi
