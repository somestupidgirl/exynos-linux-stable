...
#define CONFIG_vladutil_DOWN_RATE_LIMIT		(30000)
#define CONFIG_vladutil_UP_RATE_LIMIT		(40000)
#define CONFIG_vladutil_UP_RATE_LIMIT_BC	(40000)
 ...
 * @sg_policy: vladutil policy object to compute the new frequency for.
			   ...
			   u64 time)
{
	...
	*util = boosted_cpu_util(cpu, (unsigned long)&loadcpu->walt_load);
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the vladutil governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the vladutil governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor vladutil_gov;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	struct sugov_tunables *tunables;
	int ret = 0;
	unsigned long walt_ravg_window;
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_vladutil
...
static int __init sugov_register(void)
{
	...
	return cpufreq_register_governor(&vladutil_gov);
}
