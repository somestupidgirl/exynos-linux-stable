 ...
 * changes for blu_balls: eng.stk <eng.stk@sapo.pt>
 *
 ...
 * @sg_policy: blu_balls policy object to compute the new frequency for.
...
#define DEFAULT_HISPEED_LOAD_BC 95
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the blu_balls governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the blu_balls governor rejects all other frequency scaling requests.
	 *
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	tunables->hispeed_load = DEFAULT_HISPEED_LOAD_BC;
        }
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_BLU_BALLS
...
{
	return cpufreq_register_governor(&blu_balls_gov);
}
