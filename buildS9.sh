...
printf "Building for device: starlte\n"
...
if [ ! -d $KERN_DIR/$KERNEL_NAME/starlte ]; then
  mkdir -p $KERN_DIR/$KERNEL_NAME/starlte
fi

if [ ! -d $KERN_DIR/$DTB_NAME/starlte ]; then
  mkdir -p $KERN_DIR/$DTB_NAME/starlte
fi

if [ ! -f $CUR_DIR/arch/arm64/boot/Image ] || [ ! -f $CUR_DIR/arch/arm64/boot/dtb.img ]; then
  printf "Operation completed with errors!\n"
else
  cp -vr $CUR_DIR/arch/arm64/boot/Image $KERN_DIR/$KERNEL_NAME/starlte/zImage
  cp -vr $CUR_DIR/arch/arm64/boot/dtb.img $KERN_DIR/$DTB_NAME/starlte/dtb.img
  printf "Success!\n"
fi
